...
describe Travis::SSO::Helpers do
  ...
  context 'user_class' do
    ...
    it 'defaults to OpenStruct' do
      expect(described_class.user_class).to eq(OpenStruct)
    end

    it 'picks User if available' do
      User = 42
      expect(described_class.user_class).to eq(42)
    end
  ...
  describe 'current_user' do
    ...
    it 'uses Travis::SSO::Helpers.user_class' do
      ...
      expect(described_class).to receive(:user_class).and_return(subclass)
